...
export default function CrossSignalSynthesis({ view }: CrossSignalSynthesisProps) {
  return (
    <section
      className="enter rounded-lg border border-border bg-surface p-4 md:p-5 card-depth border-l-2"
    ...
    >
      ...
      <p className="text-sm text-ink leading-relaxed font-medium mb-2">{view.through}</p>
      <p className="text-sm text-ink-muted leading-relaxed mb-4">{view.dynamics}</p>
      <ol aria-label="Signal relationships" className="flex flex-col gap-2">
        {view.edges.map((edge, i) => (
          <li key={i} className="flex flex-wrap items-center gap-1.5">
            ...
            <span className="text-xs font-mono text-ink-faint">{edge.relation}</span>
            <ArrowRight className="h-3.5 w-3.5 shrink-0 text-ink-faint" aria-hidden="true" />
            <span className="bg-surface-2 rounded-md px-2 py-0.5 text-sm font-semibold text-ink">
              {edge.to}
            </span>
