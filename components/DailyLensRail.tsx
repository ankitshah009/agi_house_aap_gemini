      ...
      {LENSES.map((lens, index) => {
        ...
        return (
          <button
            ...
            className={[
              "rounded-lg border p-3 min-h-11 text-left transition-colors duration-fast",
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <span className={`h-2 w-2 rounded-full shrink-0 ${lens.dotClass}`} aria-hidden="true" />
            ...
            <p className="text-sm font-medium text-ink leading-snug">{LENS_BY_ID[lens.id].role}</p>
          </button>
        );
      })}
    </div>
