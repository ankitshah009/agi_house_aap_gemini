...
//
// Today's "3 movers" reuse Ankit's CURATED_SIGNALS (each already carries 4 lens-tailored
// cards), so "lens-tailored everywhere" is free. The NEW layer here is the cross-signal
// SYNTHESIS: per lens, how the three moves interrelate (the through-line + the dynamics
// edges that make the day legible in 60 seconds).
//
// Integration seam: when Ankit's scouting engine selects the live top-3, swap DAILY_MOVERS
// for his ranked signal ids and (optionally) regenerate LENS_SYNTHESIS — buildDailyView()
// and every component downstream stay unchanged.
...
export const DAILY_DATE = "May 30, 2026";
...
// Today's top-3 movers, in editorial priority order (mock; Ankit's engine replaces this).
...
// One directed relationship between two movers (drawn as an edge in the infographic).
